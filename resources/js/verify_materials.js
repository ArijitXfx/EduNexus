...
/**
 * Sets the value of showing_verified variable and loads the courses
...
/**
 * Fetches detail of verified courses from server using ajax request
 * @returns {undefined}
 */
...
function createCourseElement(response){
	for(var x=0;x<response.count;x++){
		...
		courseElement(id,title,authorname,subject,standard,board,language,response[x]['verified']);
	}
}
 ...
 * @param {Number} verified 1 if course is verified and 0 if non verified
 * @returns {undefined}
 */
function courseElement(id,title,authorname,subject,standard,board,language,verified){
	var url=$('#hidden').val();
	var div1 = $('<div class="col-sm-6 col-md-4"></div>');
	var div2 = $('<div class="thumbnail"></div>');
	var div3 = $('<div class="caption"></div>');
	var h3 = $('<h3>'+title+'</h3>');
	// var p01 = $('<p><a href="'+video+'" target="_blank">Click to visit course video</a></p>'); 
	// var p02 = $('<p><a href="'+url+'/'+qabank+'" download>Download Question & Answer Bank</a></p>')
	...
	div4.append('<a class="btn btn-danger" role="button" onclick="deleteCourse(this)">Delete</a>');
	if(verified=="0")
		div4.append('<a class="btn btn-success" role="button" onclick="verifyMaterial(this)">Verify</a>');
...
function showCourse(id){
	...
	$.ajax({
			...
			success: function(response){
				...
				if(response.id==id){
					window.open(url_val+"NGOManager/showCoursePage", '_self');
				}
			}
		});
}
...
/**
 * Fetches detail of non verified courses from server using ajax request
 * @returns {undefined}
 */
function loadNonVerifiedCourses(){
	...
	$.ajax({
			url: url_val+'/showNonVerifiedCourses',
			method: 'post',
			dataType: 'json',
			data: dataObj,
			success: function(response){
...
/**
 * Make a non verified course verified by making ajax request
 * @param {Object} val Verify Button
 * @returns {undefined}
 */
function verifyMaterial(val){
	var vol_id = $(val).closest('div').attr('id');
	//console.log(vol_id);
	var url_val = $('#hidden1').val()+"/edunexus/NGOManager";
	$.ajax({
			url: url_val+'/verifyCourse',
			method: 'post',
			dataType: 'json',
			data: {id:vol_id},
			success: function(response){
...
/**
 * Deletes a course by making ajax request
